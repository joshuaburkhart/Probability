...
model. The correlation is slight (cor = 0.6908895, R-squared = 0.4773283)
...
$$
  \textrm{wins}_{i} = 81.1713 + 5.0438\textrm{standard deviations from mean payroll}_{i} + \epsilon_{i}
$$
...
for (i in 1:N){ # loop 10000 times, storing loop iteration as i
    index <- sample(n, replace = TRUE) # store a vector of size n with values ranging from 1 to n as index
    team.boot <- teams_anl[index, ] # resampled data
    cor.boot[i] <- cor(team.boot$avg_z_pay, team.boot$avg_w_cnt) # what is x and y? The input & response variables, avg_z_pay and avg_w_cnt
    if(cor.boot[i] > orig_cor){
      gt_cor_cnt <- gt_cor_cnt + 1
    }
    # recalculate linear model estimates
    team.boot.lm <- lm(avg_w_cnt ~ avg_z_pay, data = team.boot) # what is x and y?
...
hist(cor.boot) #create histogram of correlation of bootstrapped data
observed <- cor(teams_anl$avg_z_pay, teams_anl$avg_w_cnt) # what is x and y? The input & response variables avg_z_pay and avg_w_cnt
...
observed <- summary(teams_anl_lm)$coefficients[2] # what is x and y? The input & response variables avg_z_pay and avg_w_cnt
...
\begin{aligned}
 ...
 P(Z_1)\cap P(Z_2)= P(Z_1)P(Z_2)\texrm{ from Def. 2.4.2 (p. 73)}
...
\begin{aligned}
 ...
 P(H_1)\cap P(H_2)\cap P(H_3)= P(H_1)P(H_2)P(H_3)\texrm{ from Def. 2.4.2 (p. 73)}
